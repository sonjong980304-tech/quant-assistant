...
- **웹(`web/app.py`)**: **계층형 멀티에이전트** 구조. 총괄 에이전트가 한국주식/
...
- **기본 화면은 멀티턴 대화**(`/`, `/chat` → `chat.html`)입니다. 같은 총괄 에이전트를
  ...
  이어갈 수 있습니다(자세한 동작은 [멀티턴 대화](#멀티턴-대화-chat) 절 참고). 
...
직접 못 쓰게 하고 사전검증된 26개 프리미티브만 JSON으로 조립하게 해 안전성을 확보하고,
