...
**Problem** — 한국 주식은 아직 팩터형 초과수익 기회가 상대적으로 오래 남아있는 시장이지만
([왜 한국 주식인가](#왜-한국-주식인가) 참고), 개인 투자자가 이를 SQL·파이썬 없이 체계적으로
검증하기는 어렵습니다. 재무제표는 정정공시·관리종목 이력 같은 시점별 함정이 있고, 백테스트는
생존편향·미래참조·공매도 같은 실수를 저지르기 쉬운데 일반적인 개인용 도구는 이런 걸 자동으로
걸러주지 않습니다. 자연어로 물어볼 수 있게 만들어도, LLM 답변이 그럴듯해 보이는 것과 실제로
맞는 것은 별개의 문제입니다.

**Analyze** — 총괄 에이전트가 질문을 도메인(한국주식/매크로/백테스트)으로 라우팅하고 결과를
검증·재시도하는 **계층형 멀티에이전트 구조**로 설계했습니다. 백테스트는 LLM이 파이썬 코드를
직접 못 쓰게 하고 사전검증된 26개 Primitive JSON으로 조립하게 해 안전성을 확보하고,
생존편향·미래참조·공매도는 결정론적으로 하드차단(소프트웨어의 "컴파일 에러"에 해당),
스토리텔링·데이터스누핑처럼 해석의 여지가 있는 문제는 LLM이 판단해 참고용으로만 첨부하도록
("린트 경고") 신뢰도에 따라 나눴습니다. 재무 데이터는 "정정 전 값을 지우지 않고 이력으로 남긴
뒤, 조회 시점 기준으로 그때 유효했던 값만 고른다"는 원칙으로 정합성을 확보하기로 했습니다.
그리고 이 설계가 실제로 의도대로 동작하는지는 자체 정합성 검증만으로는 확인할 수 없고,
DART·네이버 같은 시스템 바깥의 1차 출처와 대조해야만 확인 가능하다고 판단했습니다.

**Action** — 계층형 멀티에이전트 시스템(단발 질의 `/query` + 맥락을 기억하는 멀티턴 대화
`/chat`)을 구축했고, 백테스트에는 하드차단 3종 + 소프트경고 4종을 배선했습니다. 재무 데이터는
정정공시 이력·관리종목 이력(KRX 스냅샷+DART 공시 소급복원)·종목명 변경이력·시가총액 시점별
정확화·가격 이상치 asof-국소화·거래세율 연도별 스케줄까지 6가지 정합성 장치를 넣었습니다.
QVM(퀄리티·밸류·모멘텀) 멀티팩터 전략, 4자산 몬테카를로 올웨더 포트폴리오(21.6년
walk-forward), 매크로 레짐 신호, launchd 기반 8종 자동 데이터 수집까지 부가 기능을
붙였습니다. 마지막으로, 이 시스템이 내놓는 답이 실제로 맞는지 확인할 방법이 없다는 걸
인지하고, 시가총액 상위 종목을 동적으로 뽑아 재무제표·실시간주가·차트·스크리닝·백테스트
5개 영역을 실제 시스템(`run_hierarchical`)에 물어본 뒤 DART 원문·네이버 시세·DB 재계산·
vision 판정과 대조하는 사실확인 스크립트(`scripts/eval_factcheck.py`)를 TDD로 만들어
실행했습니다. 이 과정에서 차트처럼 결과가 큰 경우 응답이 조용히 실패하던 진짜 시스템 버그
(자식 프로세스가 결과를 다 만들어놓고도 부모에게 못 넘겨 서로 무한 대기하는 멀티프로세싱
파이프 교착상태)를 발견해 근본 원인을 고쳤고, 재발 방지 회귀 테스트도 추가했습니다.

**Result** — 아래 36개 사실확인 항목 전부(100%)가 시스템 스스로의 주장이 아니라 외부
1차 소스 재조회 또는 DB 직접 재계산과 대조해 통과를 확인한 결과입니다. 이 프로젝트가
실제로 신뢰할 만하게 동작하는지를 보여주는 증거입니다.
- 재무제표 10종목(영업이익 등) — DART 원문 재조회 대비 **10/10 일치**(±1% 오차 허용)
- 실시간 주가 10종목 — 네이버 증권 재조회 대비 **10/10 완전일치**
- 스크리닝(PER·PBR·ROE 등) 10케이스 — DB 직접 재계산 대비 **10/10 일치**
- 차트 5건(근거 데이터 일치 + 이미지 자체의 시각 품질 vision 판정) — **5/5 통과**
...
- **한국은 그 기회가 더 오래·더 강하게 남아 있는 경향이 있다.** 한국 시장은 기관·퀀트
  참여자 비중이 상대적으로 낮고, 개인투자자 비중이 거래대금의 60~70%대로 높습니다. 팩터
  전략을 체계적으로 돌리는 참여자가 적을수록, 저PBR(장부가 대비 저평가)·모멘텀 같은
  **팩터형 초과수익**이 재정거래로 덜 지워지고 더 오래 유지되는 경향이 있습니다.
...
- **4분기 순이익 57% 오염 버그 수정** — "효성화학 PER이 0.5배대로 너무 낮다"는 의문 하나를 DART 원본 공시까지 대조해 파고든 끝에, 4분기 실적이 분기값이 아니라 연간누적값 그대로 저장되는 ingest 버그를 찾아냈습니다. 전체 3,924종목의 57%(2,235종목)가 영향을 받고 있었고, 재계산 후 효성화학 TTM 순이익이 +4,755억(버그)에서 +134억(정상)으로 정정됐습니다.
...
- **특정 자산배분 전략을 자동으로 추적하고 싶은 사람**: 올웨더(4자산 몬테카를로) 같은 전략의 목표 비중을 매달 자동 재계산하고 텔레그램으로 알림받고 싶은 사람.
...
| 파마프렌치 팩터 조회 | Ken French Data Library 팩터(Mkt-RF/SMB/HML/RMW/CMA/RF/모멘텀)를 CLI에서 온디맨드 조회(저장 안 함). |
| 올웨더 포트폴리오 | 4자산(QQQ/삼성전자/TLT/ACE KRX금현물) 몬테카를로 비중최적화(종목당 10~45% 상하한) + 21년+ walk-forward 백테스트, 매달 자동 리밸런싱+텔레그램 알림. |
...
예전 6노드 파이프라인(`src/legacy/graph/`, `cli.py`만 사용, 웹은 전혀 호출 안 함)은 진짜로 노드가 여러 개입니다:
| 노드 | 역할 |
|---|---|
| `refine` | 질문 정제(LLM) — 절대금액(원/달러 혼동 위험) 질문이면 실시간 환율을 질문 텍스트에 직접 첨부 |
| `wiki_check` | 과거 질의 기록(`wiki` 테이블) 조회 — 유사도 매칭 기반 캐시 재사용 기능은 폐기돼 지금은 하위호환용 |
| `router` | route(sql\|pipeline) 판정 + '오늘 종가' 류 질문이면 실시간 시세를 DB에 먼저 보장 |
| `sql_gen` | route에 따라 SQL 또는 프리미티브 파이프라인 JSON을 LLM이 생성 |
| `wiki_save` | 성공한 질의를 `wiki` 테이블에 기록 |
| `execute` | 생성된 SQL·파이프라인을 읽기전용 연결로 실제 실행 |
| `eval`(선택, `--eval`) | Layer1(정답셋 일치)·Layer2(LLM 채점)·Layer3(실행가능성) 3층 평가 수행 |
