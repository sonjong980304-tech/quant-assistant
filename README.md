...
위 36문항 평가를 gpt-5.4-mini로 한 번 돌릴 때 실제로 드는 토큰·비용을, OpenAI 응답의
usage 필드를 직접 캡처해 항목별로 측정했습니다.
...
| 차트 | 5 | 7.0회 | $0.0169 |
| 스크리닝 | 10 | 5.0회 | $0.0118 |
| 백테스트 pytest | 1 | 0회(LLM 미사용) | $0 |

차트 질문이 가장 비쌉니다 — 차트를 그리는 파이썬 코드를 생성한 뒤, 완성된 이미지를 다시
vision으로 판정하는 2단계를 거치기 때문입니다. 스크리닝은 호출 횟수는 재무제표와 같지만
종목 랭킹표 전체를 프롬프트에 담아 입력 토큰이 훨씬 큽니다. (가격 기준: OpenAI 공식
gpt-5.4-mini — 입력 $0.75/100만 토큰, 출력 $4.50/100만 토큰.)
