...
프롬프트 입력 시, **총괄 에이전트 → (병렬) 활성 도메인 에이전트 3종(한국·매크로·백테스트) → 총괄 에이전트의 정합성 검증 → (차트 요청 시) 차트 에이전트** 순서로 흐릅니다. 아래
...
flowchart TB
    ...
    VERIFY{{"총괄 에이전트: 원 질문 ↔ 도메인 결과 정합성 검증<br/>불일치 시 최대 2회 재시도"}}
...
**Problem** — 한국 주식은 아직 팩터형 초과수익 기회가 상대적으로 오래 남아있는 시장이지만, 개인 투자자가 이를 SQL·Python 없이 체계적으로
...
QVM(퀄리티·밸류·모멘텀) 멀티팩터 전략, 7자산 몬테카를로 올웨더 포트폴리오(19년+
walk-forward, 매달 자동으로 늘어남), 매크로 레짐 신호, launchd 기반 8종 자동 데이터 수집까지 부가 기능을
붙였습니다. 
...
**Result** — 로컬 LLM(EXAONE·Qwen) vs GPT 정확도·속도를 평가하였습니다. 35개 사실확인 항목을 만들어
...
| **종합(35건)** | **34/35 (97.1%)** | **30/35 (85.7%)** | **27/35 (77.1%)** |
| 평균응답시간(35건) | 10.4s | 74.2s | 152.5s |

\*로컬LLM의 경우 차트 판정 로직이 불가능합니다.(이미지을 볼 수 있는 기능이 없음) 또한, 로컬
...
이 실측 과정에서 버그를 발견했습니다. 4분기 실적이 분기값이 아니라 연간누적값 그대로
...
위 문항 평가를 활용해 GPT모델의 토큰·비용을 LangSmith로 측정했습니다.

| 항목 | 문항수 | 문항당 평균 LLM 호출 | 문항당 평균 비용 |
|---|---|---|---|
| 재무제표 | 10 | 5.0회 | $0.0026 |
| 실시간 주가 | 10 | 9.6회 | $0.0087 |
| 차트 | 5 | 7.0회 | $0.0169 |
| 스크리닝 | 10 | 5.0회 | $0.0118 |
...
| 올웨더 포트폴리오 | 7자산(QQQ/삼성전자/TLT/ACE KRX금현물/IEF/TIP/BIL) 몬테카를로 비중최적화(종목당 10~45% 상하한 + MDD -20% 제약) + 19년+ walk-forward 백테스트, 매달 자동 리밸런싱+텔레그램 알림. |
...
| `exec_fallback.py` | 정형 경로 재시도(기본 2회) 소진 후 딱 한 번 시도하는 자유 코드 최후 폴백(SELECT 1개+파이썬 가공 1개). |
...
#### 백테스트 검증 배선 (`src/agents/backtest_verification.py` → `src/backtest/auditor.py`)
실제 판정 함수(하드차단 3종·소프트경고 4종)는 전부 `src/backtest/auditor.py`에 정의돼
있습니다. `src/agents/backtest_verification.py`는 그 판정 로직을 한 줄도 수정하지 않고
새 계층형 아키텍처의 백테스트 도메인 에이전트가 쓸 수 있게 감싸는 얇은 배선(wiring)
모듈일 뿐입니다.
...
| 도메인 | `domain_kr.py` | 국내 종목. 질문을 단일종목/다중종목(비교질문)/스크리닝(조건검색) 3갈래로 나눕니다. 단일·다중종목 경로에서는 다시 `classify_intent`가 질문을 **재무·순수 시세·기술지표** 세 축으로 독립적으로 판단합니다(LLM 우선, 실패 시 키워드 폴백). 재무 축이 함께 켜지면 별도 파일 `data_financial.py`가 추가로 호출되지만, 순수 시세·기술지표 두 축은 애초에 같은 파일 `data_price_kr.py`가 호출 한 번으로 함께 처리합니다 — 기술지표(이동평균/RSI/MACD/볼린저)는 별도로 조회하는 게 아니라, `data_price_kr.py`가 시세를 가져오면서 그 위에 `compute_technical_indicator`(TA-Lib, `src/backtest/primitives.py`)로 얹어 계산합니다. 스크리닝은 LLM이 "PER 낮은 순 10개"처럼 **조건(criteria)·개수(top_n)만 JSON으로 뽑고**, 실제 정렬·계산은 `get_cross_section`+`combine`이 결정론적으로 수행합니다(LLM이 숫자를 직접 계산하지 않음 — 계산은 항상 코드가, 판단만 LLM이). 종목명 인식은 "SK하이닉스"처럼 공식명 전체 언급뿐 아니라 "하이닉스"같이 그룹·지주 접두어(SK/LG/삼성 등)를 생략한 구어체도, company 테이블에 실재하는 회사일 때만 후보로 인정하는 방식으로 지원합니다. |
...
| 데이터 | `data_price_kr.py` / `data_financial.py` | 최하위 계층. 도메인 에이전트가 필요로 하는 주가/재무 데이터를 실제로 조회해줍니다. `data_price_kr.py`는 DB에 직접 `conn.execute()`를 쓰지 않고 **반드시 `exec_runtime.execute_sql`을 경유**합니다 — 아래 "SQL/Python 실행" 절의 안전장치(읽기전용 연결, 스키마 사전검증, 타임아웃)를 통과시키기 위함입니다. `data_financial.py`는 (호출부가 이미 `connect_readonly`로 넘겨준 연결을 그대로 쓰므로) `conn.execute()`를 직접 호출하며, `resolve_metric`이 DART/FnGuide 두 소스 중 어느 쪽 값인지, 어느 분기 값인지까지 함께 반환합니다. |

#### SQL/Python은 실행과정

이 프로젝트는 "LLM이 만든 코드를 실행"하는 경로가 성격이 다른 **두 종류**로 나뉩니다. 왜
나뉘어 있는지: 같은 macOS 머신에 실거래 자동매매 봇(`quant_trader`)이 함께 떠 있기 때문에,
LLM이 만든 신뢰할 수 없는 코드가 그 프로세스나 데이터베이스에 영향을 줄 수 없도록 하였습니다.
...
같은 이유로 계속 실패합니다. `answer_with_verification`이 **정확히 max_retries회(기본 2회)**
재시도한 뒤에도 실패하면, 그 다음엔 같은 검증을 다시 걸지 않고 딱 한 번 이 폴백을 시도합니다: LLM이 (1)
...
백테스트가 믿을 만하려면 "그 시점에 실제로 존재했던 숫자"를 재현해야 합니다(미래정보 참조·생존편향 방지). 이를 위해 한국 시장 데이터에 아래 6가지 정합성 장치를 넣었습니다. 공통 원칙은 **"원본을 지우지 않고 이력으로 남긴 뒤, 조회 시점 기준으로 그때 유효했던 값만 고른다"** 입니다.
...
```
세션에 데이터가 없다(첫 턴)
  └─ 신규 조회: answer_with_verification 그대로 실행 (SQL 재조회 + 검증 + 2회 재시도 +
...
곡선 전체가 커버하는 기간**입니다(2007-05-31부터 시작 — 7자산 중 가장 늦게 상장된
BIL의 상장일이 교집합 시작점을 결정합니다, 매달 배치가 돌 때마다 최신 종가까지
자동으로 늘어남 — 2026-07 기준 약 19.1년). 이건 알고리즘이 **매달 리밸런싱
...
- ACE KRX금현물(411060.KS): 2021년 말에야 상장돼 그 이전 데이터가 없습니다. `GLD`
  ...
  실보유 구간은 항상 진짜 데이터가 우선입니다. GLD 1주와 411060.KS 1주가 담는 금의
  양(단위)이 서로 달라 단순히 이어붙이기만 하면 스플라이스 시점의 레벨이 실측상 약
  19.8배 어긋나(하루 만에 -95%로 보이는 가짜 폭락) `pct_change` 기반 계산
  (`montecarlo.py`의 공분산·수익률, `backtest.py`의 리밸런싱 실현수익률)을 오염시킵니다.
  그래서 실데이터 시작 직전 합성값 대비 실데이터 첫 값의 비율(scale)을 구해 합성
  구간 전체에 곱해 레벨을 맞춥니다 — 상수를 곱하는 것이므로 합성 구간 내부의 일별
  수익률(추세)은 그대로 보존됩니다.
...
| `financials_revision` | 재무제표 정정공시 이력 — 정정 전/후 버전을 각각 보존(`rcept_no` UNIQUE로 재수집 멱등), look-ahead 방지용 |
...
| `kr_admin_status_history` | 관리종목·거래정지 과거 이력(DART 공시 기반 소급 복원) — 백테스트 look-ahead 경로(`_admin_halt_status_at`)에 배선 |
| `kr_stock_changes` | 종목코드별 상호/업종/액면가 변경 이력(pykrx) |
...
| GET | `/api/macro/vkospi` | VKOSPI(코스피200 변동성지수) 최신값 — 참고 표시 전용, 종합신호 판정엔 미반영 |
...
- **"매출총이익" 등 한국어 별칭 누락으로 조회가 안 되던 버그 수정** — `gross_profit`(매출총이익)은 DART 표준계정으로 실제 데이터도 다 있었는데, 한국어 별칭 사전에 "매출총이익" 자체가 등록돼 있지 않아 "매출"(매출액) 별칭에 부분문자열로 걸려 엉뚱한 값이 조회되는 걸 확인했습니다. 같은 원인으로 매출원가/판관비/이자비용/영업활동현금흐름/감가상각비/발행주식수도 전혀 조회되지 않고 있어 함께 별칭을 등록해 근본 해결했습니다.
