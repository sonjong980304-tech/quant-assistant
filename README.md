...
## 이 프로젝트가 실제로 신경 써서 만들어졌다는 증거

- 🐛 **무한대기 교착상태 버그 발견·수정** — 차트처럼 결과가 큰 경우 응답이 조용히 실패하던 진짜 시스템 버그(자식 프로세스가 결과를 다 만들어놓고도 부모에게 못 넘겨 서로 무한 대기하는 멀티프로세싱 파이프 교착상태)를 사실확인 스크립트 실행 중 발견해 근본 원인을 고쳤고, 재발 방지 회귀 테스트도 추가했습니다.
...
- ⚠️ **gpt-5.5 100% 실패 버그 수정** — OpenAI 추론모델(gpt-5.5)이 temperature=0을 거부해 모든 질의가 100% 실패하는 걸 실제 API 호출로 확인하고, 설정값을 4단계로 순차 완화하는 재시도 로직으로 근본 해결했습니다.
...
- [설치와 빠른 시작](#설치와-빠른-시작)
- [무엇을 만들었나 — 배경과 설계 판단 (PAAR)](#무엇을-만들었나--배경과-설계-판단-paar)
- [기능 둘러보기](#기능-둘러보기)
- [아키텍처 심층](#아키텍처-심층)
...
## 무엇을 만들었나 — 배경과 설계 판단 (PAAR)

<details>
<summary>펼쳐서 보기 — Problem / Analyze / Action / Result 요약 + 왜 한국 주식인가</summary>
