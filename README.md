...
| 파일 | 역할 |
|---|---|
| `supervisor.py` | 총괄 — 라우팅(`route_question`)→도메인 실행(`dispatch_domains`)→정합성 검증(`verify_answer`)→최대 2회 재시도→종합결론(`answer_with_verification` 하나로 통합). 차트 요청 시 `_build_charts`(결정론적 3케이스) 우선 시도 후 빈 결과면 `chart_agent`로 폴백. |
