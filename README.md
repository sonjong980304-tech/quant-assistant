...
### 로컬 LLM(EXAONE·Qwen) vs GPT 정확도·속도 비교

위 factcheck 36개 항목을 로컬 모델(EXAONE 3.5 7.8B, Qwen2.5-coder 7B, 둘 다 Ollama로 무료
구동)로도 그대로 돌려 GPT(gpt-5.4-mini)와 비교했습니다. `scripts/eval_factcheck.py --model
<ollama모델명>`으로 동일하게 재현할 수 있습니다.
...
- **재무제표·주가는 세 모델 모두 완벽** — DB에서 값을 그대로 읽어오는 결정론적 경로라 모델
  성능과 무관합니다.
- **차트는 로컬 모델 둘 다 사실상 전멸**했고, GPT만 정상 동작했습니다.
- **스크리닝 정확도**는 GPT 100% > Qwen 90% > EXAONE 60% 순이었고, EXAONE은 존재하지 않는
  종목코드를 만들어내는 할루시네이션이 반복 관찰됐습니다.
- **응답 속도**는 GPT가 압도적으로 빨랐고, 로컬 모델 중에서는 Qwen이 EXAONE보다 뚜렷하게
  빨랐습니다.
