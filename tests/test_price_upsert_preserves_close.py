"""pykrx 기반 주가 수집 3곳(price_live.py, backfill_prices.py, update_prices.py)이
이미 저장된 close(네이버의 수정주가일 수 있음)를 pykrx의 미조정 원본 종가로
덮어쓰지 않는지 검증한다.

배경: naver_prices.py의 문서화된 원칙(AC3)은 "네이버가 close의 source of truth"
(액면분할 반영된 수정주가)이다. 그런데 pykrx 기반 수집 곳들이 전부
