...
ORDER BY per ASC LIMIT 1""", "tags": "PER,업종"},
    ...
    # ---------- US 단일시장 + 한미 혼합(.omc/specs/brainstorming-us-nl-sql-integration.md) ----------
    # AAPL/MSFT/TSLA/GOOGL/AMZN 5종목만 실데이터 백필(2026-07-13, yfinance) — 이 5종목 대상만 작성.
    {"id": 51, "question": "애플 최신 종가 알려줘",
     "sql": """SELECT name, 'USD' AS currency, ROUND(p.close,2) AS close FROM us_prices p
JOIN us_company c ON c.stock_code=p.stock_code
WHERE p.stock_code='AAPL' AND p.date=(SELECT MAX(date) FROM us_prices WHERE stock_code='AAPL')""",
     "tags": "US,주가"},
    {"id": 52, "question": "테슬라 최신 종가 알려줘",
     "sql": """SELECT name, 'USD' AS currency, ROUND(p.close,2) AS close FROM us_prices p
JOIN us_company c ON c.stock_code=p.stock_code
WHERE p.stock_code='TSLA' AND p.date=(SELECT MAX(date) FROM us_prices WHERE stock_code='TSLA')""",
     "tags": "US,주가"},
    {"id": 53, "question": "마이크로소프트 최신 종가 알려줘",
     "sql": """SELECT name, 'USD' AS currency, ROUND(p.close,2) AS close FROM us_prices p
JOIN us_company c ON c.stock_code=p.stock_code
WHERE p.stock_code='MSFT' AND p.date=(SELECT MAX(date) FROM us_prices WHERE stock_code='MSFT')""",
     "tags": "US,주가"},
    {"id": 54, "question": "PER이 가장 낮은 미국 종목",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
  SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
  WHERE item_key='Net Income' AND period_type='quarterly'
    AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.ni_ttm>0 ORDER BY per ASC LIMIT 1""",
     "tags": "US,PER"},
    {"id": 55, "question": "PER이 가장 높은 미국 종목",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
  SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
  WHERE item_key='Net Income' AND period_type='quarterly'
    AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.ni_ttm>0 ORDER BY per DESC LIMIT 1""",
     "tags": "US,PER"},
    {"id": 56, "question": "애플 PER 알려줘",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
  SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
  WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='AAPL'
    AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='AAPL' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code WHERE f.ni_ttm>0""",
     "tags": "US,PER"},
    {"id": 57, "question": "구글 PER 알려줘",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
  SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
  WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='GOOGL'
    AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='GOOGL' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code WHERE f.ni_ttm>0""",
     "tags": "US,PER"},
    {"id": 58, "question": "아마존 매출액(연간) 알려줘",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(f.item_value,2) AS revenue FROM us_financials f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.stock_code='AMZN' AND f.item_key='Total Revenue' AND f.period_type='annual'
  AND f.as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='AMZN' AND item_key='Total Revenue' AND period_type='annual')""",
     "tags": "US,매출"},
    {"id": 59, "question": "구글 영업이익(연간) 알려줘",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(f.item_value,2) AS operating_income FROM us_financials f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.stock_code='GOOGL' AND f.item_key='Operating Income' AND f.period_type='annual'
  AND f.as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='GOOGL' AND item_key='Operating Income' AND period_type='annual')""",
     "tags": "US,영업이익"},
    {"id": 60, "question": "테슬라 순이익(연간) 알려줘",
     "sql": """SELECT c.name, 'USD' AS currency, ROUND(f.item_value,2) AS net_income FROM us_financials f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.stock_code='TSLA' AND f.item_key='Net Income' AND f.period_type='annual'
  AND f.as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='TSLA' AND item_key='Net Income' AND period_type='annual')""",
     "tags": "US,순이익"},
    {"id": 61, "question": "애플 ROE 알려줘",
     "sql": """SELECT c.name, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
  SELECT stock_code, SUM(item_value) ttm FROM us_financials
  WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='AAPL'
    AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='AAPL' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
  GROUP BY stock_code) ni
JOIN (SELECT stock_code, item_value eq FROM us_financials
      WHERE item_key='Stockholders Equity' AND period_type='quarterly' AND stock_code='AAPL'
        AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='AAPL' AND period_type='quarterly')) e
  ON e.stock_code=ni.stock_code
JOIN us_company c ON c.stock_code=ni.stock_code WHERE ni.ttm>0 AND e.eq>0""",
     "tags": "US,ROE"},
    {"id": 62, "question": "마이크로소프트 영업이익률 알려줘",
     "sql": """SELECT c.name, ROUND(f.op*100.0/f.rev,2) AS operating_margin FROM (
  SELECT stock_code,
    MAX(CASE WHEN item_key='Total Revenue' THEN item_value END) rev,
    MAX(CASE WHEN item_key='Operating Income' THEN item_value END) op
  FROM us_financials WHERE period_type='quarterly' AND stock_code='MSFT'
    AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='MSFT' AND period_type='quarterly')
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code WHERE f.rev>0""",
     "tags": "US,영업이익률"},
    {"id": 63, "question": "구글 매출총이익률 알려줘",
     "sql": """SELECT c.name, ROUND(f.gp*100.0/f.rev,2) AS gross_margin FROM (
  SELECT stock_code,
    MAX(CASE WHEN item_key='Total Revenue' THEN item_value END) rev,
    MAX(CASE WHEN item_key='Gross Profit' THEN item_value END) gp
  FROM us_financials WHERE period_type='quarterly' AND stock_code='GOOGL'
    AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='GOOGL' AND period_type='quarterly')
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code WHERE f.rev>0""",
     "tags": "US,매출총이익률"},
    {"id": 64, "question": "영업이익률이 가장 높은 미국 종목",
     "sql": """SELECT c.name, ROUND(f.op*100.0/f.rev,2) AS operating_margin FROM (
  SELECT stock_code,
    MAX(CASE WHEN item_key='Total Revenue' THEN item_value END) rev,
    MAX(CASE WHEN item_key='Operating Income' THEN item_value END) op
  FROM us_financials WHERE period_type='quarterly'
    AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE period_type='quarterly')
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code
WHERE f.rev>0 AND f.op IS NOT NULL ORDER BY operating_margin DESC LIMIT 1""",
     "tags": "US,영업이익률"},
    {"id": 65, "question": "테슬라 부채비율 알려줘",
     "sql": """SELECT c.name, ROUND(f.liab*100.0/f.eq,2) AS debt_ratio FROM (
  SELECT stock_code,
    MAX(CASE WHEN item_key='Total Liabilities Net Minority Interest' THEN item_value END) liab,
    MAX(CASE WHEN item_key='Stockholders Equity' THEN item_value END) eq
  FROM us_financials WHERE period_type='quarterly' AND stock_code='TSLA'
    AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='TSLA' AND period_type='quarterly')
  GROUP BY stock_code) f
JOIN us_company c ON c.stock_code=f.stock_code WHERE f.eq>0""",
     "tags": "US,부채비율"},
    {"id": 66, "question": "삼성전자와 애플 중 PER 낮은 곳",
     "sql": """SELECT name, per FROM (
  SELECT c.name, ROUND(p.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(CASE WHEN account_key='net_income' THEN amount END) ni_ttm
    FROM financials WHERE account_key='net_income'
      AND quarter IN (SELECT DISTINCT quarter FROM financials ORDER BY quarter DESC LIMIT 4)
    GROUP BY stock_code) f
  JOIN prices p ON p.stock_code=f.stock_code AND p.date=(SELECT MAX(date) FROM prices)
  JOIN company c ON c.stock_code=f.stock_code
  WHERE c.name='삼성전자' AND f.ni_ttm>0
  UNION ALL
  SELECT c.name, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
    WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='AAPL'
      AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='AAPL' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
    GROUP BY stock_code) f
  JOIN us_company c ON c.stock_code=f.stock_code WHERE f.ni_ttm>0
     ...
     "tags": "혼합,PER"},
    {"id": 67, "question": "삼성전자와 마이크로소프트 중 ROE 높은 곳",
     "sql": """SELECT name, roe FROM (
  SELECT c.name, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
    SELECT stock_code, SUM(amount) ttm FROM financials
    WHERE account_key='controlling_net_income'
      AND quarter IN (SELECT DISTINCT quarter FROM financials ORDER BY quarter DESC LIMIT 4)
    GROUP BY stock_code) ni
  JOIN (SELECT stock_code, amount eq FROM financials
    ...
    ON e.stock_code=ni.stock_code
  JOIN company c ON c.stock_code=ni.stock_code
  WHERE c.name='삼성전자' AND ni.ttm>0
  UNION ALL
  SELECT c.name, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
    SELECT stock_code, SUM(item_value) ttm FROM us_financials
    WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='MSFT'
      AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='MSFT' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
    GROUP BY stock_code) ni
  JOIN (SELECT stock_code, item_value eq FROM us_financials
        WHERE item_key='Stockholders Equity' AND period_type='quarterly' AND stock_code='MSFT'
          AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='MSFT' AND period_type='quarterly')) e
    ON e.stock_code=ni.stock_code
  JOIN us_company c ON c.stock_code=ni.stock_code WHERE ni.ttm>0
) ORDER BY roe DESC LIMIT 1""",
     "tags": "혼합,ROE"},
    {"id": 68, "question": "SK하이닉스와 테슬라 중 PER 낮은 곳",
     "sql": """SELECT name, per FROM (
  SELECT c.name, ROUND(p.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(CASE WHEN account_key='net_income' THEN amount END) ni_ttm
    FROM financials WHERE account_key='net_income'
  ...
  JOIN prices p ON p.stock_code=f.stock_code AND p.date=(SELECT MAX(date) FROM prices)
  JOIN company c ON c.stock_code=f.stock_code
  WHERE c.name='SK하이닉스' AND f.ni_ttm>0
  UNION ALL
  SELECT c.name, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
    WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='TSLA'
      AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='TSLA' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
    GROUP BY stock_code) f
  JOIN us_company c ON c.stock_code=f.stock_code WHERE f.ni_ttm>0
) ORDER BY per ASC LIMIT 1""",
     "tags": "혼합,PER"},
    {"id": 69, "question": "SK하이닉스와 아마존 중 PER 낮은 곳",
     "sql": """SELECT name, per FROM (
  SELECT c.name, ROUND(p.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(CASE WHEN account_key='net_income' THEN amount END) ni_ttm
    FROM financials WHERE account_key='net_income'
  ...
  SELECT c.name, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
    SELECT stock_code, SUM(item_value) ni_ttm FROM us_financials
    WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='AMZN'
      AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials WHERE stock_code='AMZN' AND period_type='quarterly' ORDER BY as_of_date DESC LIMIT 4)
    GROUP BY stock_code) f
  JOIN us_company c ON c.stock_code=f.stock_code WHERE f.ni_ttm>0
) ORDER BY per ASC LIMIT 1""",
     "tags": "혼합,PER"},
    {"id": 70, "question": "삼성전자와 구글 중 ROE 높은 곳",
     "sql": """SELECT name, roe FROM (
  SELECT c.name, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
    SELECT stock_code, SUM(amount) ttm FROM financials
    WHERE account_key='controlling_net_income'
  ...
  SELECT c.name, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
    ...
    GROUP BY stock_code) ni
  JOIN (SELECT stock_code, item_value eq FROM us_financials
        WHERE item_key='Stockholders Equity' AND period_type='quarterly' AND stock_code='GOOGL'
          AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials WHERE stock_code='GOOGL' AND period_type='quarterly')) e
    ON e.stock_code=ni.stock_code
  JOIN us_company c ON c.stock_code=ni.stock_code WHERE ni.ttm>0
) ORDER BY roe DESC LIMIT 1""",
     "tags": "혼합,ROE"},
