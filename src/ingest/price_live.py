...
# --------------------------------------------------------------------------
# US 버전 — 위 KR 로직과 동일 패턴, 데이터 소스만 pykrx→yfinance, 테이블만 prices→us_prices.
# .omc/specs/brainstorming-us-nl-sql-integration.md AC5 참고.
# --------------------------------------------------------------------------
def _has_today_us(conn, code: str, today: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM us_prices WHERE stock_code=? AND date=? LIMIT 1", (code, today)
    ).fetchone()
    return row is not None


def _fetch_latest_close_us(code: str, lookback_period: str = "5d"):
    """최근 5거래일 범위에서 가장 최근 종가 1건(yfinance)."""
    import yfinance as yf  # 지연 import — us_prices.py의 기존 패턴과 동일

    try:
        df = yf.Ticker(code).history(period=lookback_period)
    except Exception:  # noqa: BLE001 — 네트워크/차단은 격리
        return None
...
def ensure_live_prices_us(conn, codes, on: date | None = None, fetch_fn=None) -> dict:
    """codes(US 티커) 각각에 대해 오늘 종가를 us_prices에 보장(없으면 yfinance 실시간 1건).

    fetch_fn(code)->close|None 주입 가능(테스트용, 기본은 _fetch_latest_close_us).
    반환: {"fetched": 신규 호출 건수, "cached": 이미 있던 건수, "today": 'YYYY-MM-DD'}
    """
    fetch_fn = fetch_fn or _fetch_latest_close_us
    on = on or date.today()
    today = on.strftime("%Y-%m-%d")
    fetched = cached = 0
    for code in codes:
        if _has_today_us(conn, code, today):
            cached += 1
            continue
        close = fetch_fn(code)
        if close is None:
            continue
        # market_cap은 시가총액 API를 별도로 안 부르므로 NULL이지만, 이미 다른 경로로
        # 채워진 값이 있으면 지우지 않는다(ensure_live_prices/C-9와 동일한 ON CONFLICT 패턴).
        conn.execute(
            "INSERT INTO us_prices(stock_code, date, close) VALUES (?,?,?) "
            "ON CONFLICT(stock_code, date) DO UPDATE SET close=excluded.close",
            (code, today, close),
        )
        fetched += 1
    if fetched:
