...
# ---------------------------------------------------------------------------
# VKOSPI 밴드 분류 — VIX와 산출 방식(옵션 내재변동성 지수)이 동일해 같은 구간을
# 그대로 적용한다. 참고 표시 전용(VKOSPI는 _INDICATORS 밖이라 종합신호 미반영).
#   <15 안정 | 15-20 보통 | 20-30 경계 | >=30 공포
# ---------------------------------------------------------------------------
def classify_vkospi_band(value: float) -> str:
    return classify_vix_band(value)
