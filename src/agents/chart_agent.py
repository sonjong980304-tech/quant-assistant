...
차트 종류 판정은 전적으로 LLM에 위임한다. 예전엔 supervisor.py의 결정론적 패턴매칭
(_build_charts, 산점도/분위수막대/히스토그램/시계열만 인식)을 먼저 시도하고 안 맞을 때만
이 LLM 경로로 폴백했는데, 그 패턴매칭이 숫자 필드 1개짜리 스크리닝 결과에서 같은 필드를
x·y에 둘 다 배정해 "pbr vs pbr 산점도"를 그리는 오판을 냈다(실사용 스크린샷 재현). 그래서
결정론 1단계를 아예 제거하고, wants_chart면 무조건 이 모듈이 질문·데이터에 맞는 종류를
자유롭게 골라 그린다(막대/선/산점도/히스토그램/파이/박스플롯 등 matplotlib 전체).
...
def build_chart_freeform(
    ...
    원칙, supervisor.py answer_with_verification의 차트 배선 주석과 동일).
    """
    if llm_fn is None:
        return None
