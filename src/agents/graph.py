...
최종 결론/원본 도메인 결과는 이벤트가 아니라 그래프 최종 상태(run_hierarchical)로 얻는다.
    ...
    def _worker() -> None:
        try:
            ...
            if steps is not None:
                init["steps"] = steps
            for _ in graph.stream(init, stream_mode="values"):
                pass  # 최종 상태는 run_hierarchical의 몫 — 여기선 진행 이벤트만 흘려보낸다.
