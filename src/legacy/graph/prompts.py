...
[미국(US) 종목 — 티커/영문명/한글명("애플"="Apple"="AAPL") 어느 쪽으로 물어도 이 테이블들을 쓴다]
- us_company(stock_code, name, exchange, sector, market_cap)  · stock_code=티커(예:'AAPL'), market_cap 단위=달러
- us_prices(stock_code, date, open, high, low, close, volume)  · close=종가(달러), 최신일=(SELECT MAX(date) FROM us_prices)
- us_financials(stock_code, as_of_date, period_type, statement_type, item_key, item_value)
  · period_type='quarterly'|'annual', statement_type='income_stmt'|'balance_sheet'|'cashflow'
  · item_key는 yfinance 원본 영문 항목명 그대로(예: 'Total Revenue') — 반드시 아래 매핑만 쓸 것(존재하지 않는 item_key 금지):
    매출=Total Revenue, 매출원가=Cost Of Revenue, 매출총이익=Gross Profit, 영업이익=Operating Income,
    순이익=Net Income, 총부채=Total Liabilities Net Minority Interest, 총자본=Stockholders Equity
  · 최신 as_of_date는 종목별로 (SELECT MAX(as_of_date) FROM us_financials WHERE stock_code=... AND period_type='quarterly')
- **US 종목 결과에는 반드시 SELECT에 'USD' AS currency 컬럼을 추가한다**(원/달러 혼동 방지, 결과 표시용).
  KR 종목 결과에는 currency 컬럼을 붙이지 않는다(원화가 기본이므로).
...
- **주의: 위 정합성 가드(op<rev, ni.ttm<e.eq, ni_ttm<op_ttm 등)는 여러 회사를 비교하는 순위/스크리닝 질의에만 적용한다. 특정 회사 1곳을 지목한 단일조회 질의에는 적용하지 않는다.** 이 가드는 데이터 오류성 이상치가 순위 1위를 차지하는 것을 막기 위한 것이지, 그 회사의 실제 지표를 숨기기 위한 것이 아니다 — 애플처럼 자사주매입으로 ROE가 100%를 실제로 넘는 정상 기업도 있으므로, 단일회사 조회는 있는 그대로 보여준다.
...
Q: 테슬라 최신 종가 알려줘
A: SELECT c.name, 'USD' AS currency, ROUND(p.close,2) AS close FROM us_prices p
...
Q: 애플 ROE 알려줘
A: SELECT c.name, 'USD' AS currency, ROUND(ni.ttm*100.0/e.eq,2) AS roe FROM (
     ...
     WHERE item_key='Net Income' AND period_type='quarterly' AND stock_code='AAPL'
       AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials
                          WHERE stock_code='AAPL' AND period_type='quarterly'
   ...
   JOIN (SELECT stock_code, item_value eq FROM us_financials
         WHERE item_key='Stockholders Equity' AND period_type='quarterly' AND stock_code='AAPL'
           AND as_of_date=(SELECT MAX(as_of_date) FROM us_financials
                           WHERE stock_code='AAPL' AND period_type='quarterly')) e
     ON e.stock_code=ni.stock_code
   JOIN us_company c ON c.stock_code=ni.stock_code
   WHERE ni.ttm>0 AND e.eq>0;
Q: 애플 PER 알려줘
A: SELECT c.name, 'USD' AS currency, ROUND(c.market_cap/f.ni_ttm,2) AS per FROM (
     ...
     WHERE item_key='Net Income' AND period_type='quarterly'
       AND as_of_date IN (SELECT DISTINCT as_of_date FROM us_financials
                          ...
                          ORDER BY as_of_date DESC LIMIT 4)
     GROUP BY stock_code) f
   JOIN us_prices p ON p.stock_code=f.stock_code AND p.date=(SELECT MAX(date) FROM us_prices)
   JOIN us_company c ON c.stock_code=f.stock_code
   WHERE c.stock_code='AAPL' AND f.ni_ttm>0;
Q: 삼성전자와 애플 중 PER 낮은 곳
A: SELECT name, per FROM (
     ...
     JOIN prices p ON p.stock_code=f.stock_code AND p.date=(SELECT MAX(date) FROM prices)
     JOIN company c ON c.stock_code=f.stock_code
     WHERE c.name LIKE '%삼성전자%' AND f.ni_ttm>0
     UNION ALL
     ...
     JOIN us_company c ON c.stock_code=f.stock_code
     WHERE c.stock_code='AAPL' AND f.ni_ttm>0
   ) ORDER BY per ASC LIMIT 2;
[한미 비교 규칙] PER/PBR/ROE 등 통화 무관 비율 지표만 UNION으로 비교한다. 시가총액/매출 등
절대금액(원/달러) 비교는 이 방식으로 하지 말 것(환율 미반영 시 틀린 비교가 된다).
...
Q: 미국 종목 중 PER 낮은 10개로 2024년부터 2026년까지 매 분기 리밸런싱했을 때 S&P500 대비 백테스트 결과 보여줘
A: {{"pipeline": [
  {{"op": "run_backtest", "params": {{"start_year": 2024, "end_year": 2026, "criteria": [{{"key": "per", "direction": "low", "weight": 1.0}}], "n": 10, "rebalance": "quarterly", "market": "US"}}, "out": "bt"}}
]}}
